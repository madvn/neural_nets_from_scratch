...
class KohonenMap:
    def __init__(self, map_size, input_size, learn_rate=0.01):
        ...
        self.input_weights = np.random.random([self.map_size[0],
                                                self.map_size[1],
                                                self.input_size
                                                ])
        for i in range(self.map_size[0]):
            for j in range(self.map_size[1]):
                self.input_weights[i][j][:] /= np.linalg.norm(self.input_weights[i][j][:])


        self.lateral_weights = np.random.random(self.map_size)
        self.lateral_excitatory_mask = [[i,j] for i,j in itertools.product(np.arange(-3,4,1), repeat=2)
                                                                            if np.linalg.norm([i,j])<3]
        self.lateral_inhibitory_mask = [[i,j] for i,j in itertools.product(np.arange(-8,9,1), repeat=2)
    ...
    def _estimate_external_input(self, input):
        ...
        """
        reshaped_weights = np.reshape(self.input_weights, [
                                                            np.prod(self.map_size),
                                                            self.input_size
                                                            ])
    ...
    def train(self, input):
        ...
        for i in range(self.map_size[0]):
            for j in range(self.map_size[1]):
                self.input_weights[i][j][:] += self.learning_rate*self.eta[i][j]*np.double(input)
                self.input_weights[i][j][:] /= np.linalg.norm(self.input_weights[i][j][:])
